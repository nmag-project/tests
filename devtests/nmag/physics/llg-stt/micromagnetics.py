...
def add_landau_lifshitz(m):
    ...
    m.add_quantity([m, M_sat, H_tot, gamma_GG, alpha, norm_coeff, dmdt])

    h_total_equation = "H_tot(i) <- H_ext(i)"
    if m.quantities._by_name.has_key("H_exch"):
        h_total_equation += "+ H_exch(i)"
    if m.quantities._by_name.has_key("H_demag"):
        h_total_equation += "+ H_demag(i)"

    # Equation for the effective field H_tot
    eq_H_tot = Equation("H_tot", "%range i:3; %s;" % h_total_equation)
    ...
    m.add_computation([llg, eq_H_tot])
