...
default_state = {'nanopillar_radius':10.0, 'nanopillar_length':40.0,
                 'discretization':3.0, 'current_density':10.0,
                 'use_demag':True}
...
states = [{'nanopillar_length':l, 'current_density':j}
          for l in [30]
          for j in [10]]
...
def directory_from_state(state):
    l, j = state['nanopillar_length'], state['current_density']
...
def mesh_name_from_state(state):
    ...
    return "l%03d.nmesh.h5" % (int(round(l)))
