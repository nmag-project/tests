...
def test_gmsh_1_0_import():
    ...
    #
    f=open("sphere.msh","r")
    assert f.readline()[0:4] == "$NOD", "gmsh file 'sphere.msh' has not been written correctly"
    f.close()

    os.system("make sphere.nmesh &> test_stdoutstderr2.log")
    ...
    assert n == expect, "Number of nodes has changed. Expected %d, but got %d" % (expect,n)

    n,expect =len(mesh.simplices),677
    assert n == expect, "Number of simplices has changed. Expected %d, but got %d" % (expect,n)

    n,expect =len(mesh.simplicesregions),677
    assert n == expect, "Number of simplicesregions has changed. Expected %d, but got %d" % (expect,n)


    #for the number of links, we have seen 1687 and 1689. This seems to be non-deterministic behaviour
    #from parmetis. We thus allow a range of links.
    n,expect =len(mesh.links),"1670-1700 (1687 and 1689 have been seen)"
    assert 1670 <= n <= 1700, "Number of links has changed. Expected %s, but got %d" % (expect,n)

    n,expect = mesh.dim,3
    assert n == expect, "Number of dim has changed. Expected %d, but got %d" % (expect,n)

    n,expect =len(mesh.periodicpointindices),0
    assert n == expect, "Number of periodicpointsindices has changed. Expected %d, but got %d" % (expect,n)

    n,expect =mesh.numregions,2
    assert n == expect, "Number of regions has changed. Expected %d, but got %d" % (expect,n)

    n,expect =max(max(mesh.pointsregions)),1
    assert n == expect, "max(max(pointregions)) has changed. Expected %d, but got %d" % (expect,n)

    n,expect =min(min(mesh.pointsregions)),-1
    assert n == expect, "min(min(pointregions)) has changed. Expected %d, but got %d" % (expect,n)
