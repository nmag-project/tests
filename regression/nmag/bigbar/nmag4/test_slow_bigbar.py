...
def datafile2numpy(filename):
    ...
    for line in lines:
        bits = line.split()
        assert len(bits) == 4, \
          ("Error in '%s' -- expect 4 columns but got %d.\n"
           "Full line reads '%s'" % (filename, len(bits), line))
...
def do_differencecheck():
    ...
    t2,M2 = datafile2numpy('../results/nmag4_M.dat')

    # We expect the difference between newer nmag runs and this 'old' run to be
    # very, very small.
    #
    # To get a feel for the expected deviation, here is the data when comparing
    # with an older (nmag3) data set:
    ...
    # The reason for comparing with nmag data is thatwe have that available at
