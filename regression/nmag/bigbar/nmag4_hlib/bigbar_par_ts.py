...
if ocaml.petsc_is_mpi():
    ...
    if ocaml.petsc_mpi_nr_nodes()==2:
        distrib=[1059,1058]
    elif ocaml.petsc_mpi_nr_nodes()==1:
        distrib=[1059+1058]
    else:
...
def new_advance_time(time_si):

    time_su = nmag.simulation_units.of(time_si)
    ...
    ocaml.linalg_machine_advance_timestepper(sim._lam,'timestepper',time_su,-1)
...
if True:
    ...
    #for i in range(0, 5):
        ...
        target_time = sim.advance_time_par(time_)
    ...
    print sim.get_timers()
    print "== PARALLEL TIMESTEPPER ==="
    print ocaml.linalg_machine_get_timestepper_timings(sim._lam,"timestepper")
